...

{
    ...
    let scheduler = function () {
        if (system_suspended !== true) {
            ...
            let processes_buffer = processes.sort((a, b) => a.priority - b.priortiy);//Order processes by priority
            user_time_buffer = 0;
            let dead_process = false;
            for (let i = 0; i < processes_buffer.length; i++) {
                let process = processes_buffer[i];
                time_marker = process.run(time_marker, start_time, target_time);
                if(process.dead === true) dead_process = true;
            }
