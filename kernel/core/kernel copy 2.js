...

{
    ...
    let user_time = 0;
    let queue_sort = false;
    let threads = [];
    let sort_threads = function() {
        threads.sort((a, b) => {
            let predicted_exec_difference = (a.queued_execution + a.exec_time) - (b.queued_execution + b.exec_time);
            if(predicted_exec_difference) return predicted_exec_difference;
            return b.process.priority - a.process.priority;
        });
    }
    let scheduler = function () {
        if (system_suspended !== true) {
            ...
            // Sort threads for optimal execution
            sort_threads();
        ...
        function sleep(timeout) {
            run_kernel_api(() => {
                /*  Sleep 2.0
                    ...
                    that need high responsiveness. Things like software cursors and audio engines should run MUCH better from this.
                ...
                */
                ...
                for(let i = 1; i < threads.length - 1; i++) {
                    ...
                    if(timeout <= predicted_scheduler_time && thread_in_execution.process.priority >= thread.process.priority && i > 1) {
                        ...
                        // threads.push(thread_in_execution);
                        // queue_sort = true;
                        return;
                    }
                }
